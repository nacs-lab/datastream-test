...
function preprocess_data(data)
    return (nele=Int.(@view data[:, 1]),
            ...
            submit_cb=(@view data[:, 3]) .!= 0,
            start_cb=(@view data[:, 4]) .!= 0,
            complete_cb=(@view data[:, 5]) .!= 0,
            t=data[:, 6])
end
...
function filter_data(cb, data)
    res = (nele=Int[], nrep=Int[], t=Float64[],
           submit_cb=Bool[], start_cb=Bool[], complete_cb=Bool[])
    for i in 1:length(data.nele)
        point = (nele=data.nele[i], nrep=data.nrep[i], t=data.t[i],
                 submit_cb=data.submit_cb[i], start_cb=data.start_cb[i],
                 complete_cb=data.complete_cb[i])
        cb(point) || continue
        push!(res.nele, point.nele)
        push!(res.nrep, point.nrep)
        push!(res.submit_cb, point.submit_cb)
        push!(res.start_cb, point.start_cb)
...
function plot_nele(data)
    ...
    for i in 0:7
        ...
        start_cb = (i & 2) != 0
        complete_cb = (i & 4) != 0
        line = filter_data(x->(x.submit_cb == submit_cb && x.start_cb == start_cb &&
                               x.complete_cb == complete_cb), data)
        x = line.nele .* 4
        ...
        plot([], [], ".", ls="-", color=color, label="$((submit_cb, start_cb, complete_cb) .+ 0)")
    end
