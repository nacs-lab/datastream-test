...
function write_file(nchn, num_block_final, use_nt)
    name = "$(nchn * ncores)chn-$(num_block_final * block_size)$(use_nt ? "-nt" : "")"
    open(joinpath(dir, name * ".yaml"), "w") do io
        ...
        println(io, "final_use_nt: $(use_nt)")
        println(io, "workers:")
...
for use_nt in [true, false]
    for nchn in [1]
        ...
        while num_block_final >= num_block
            write_file(nchn, num_block_final, use_nt)
            num_block_final ÷= 4
        end
    end
end
