                              ...
                              int ncalc_single, int ncalc_double)
{
    ...
    std::string source{R"CLC(
        ...
        void compute_in_order(global float *res, float amp, float freq,
                              int ncalc_single, int ncalc_double)
        {
            {
                                         ...
                                         int ncalc_single, int ncalc_double)
        {
            ...
            if (ncalc_single > ncalc_double) {
                ...
                float val_s = get_global_id(0) % 512;
                double val_d = get_global_id(0) % 512;
                for (int i = 0; i < ncalc_double; i++) {
                    val_d = amp * native_sin(freq * val_d);
                    for (int j = 0; j < ni; j++) {
                        val_s = amp * native_sin(freq * val_s);
                    }
                }
                for (int i = 0; i < ncalc_single - ncalc_double * ni; i++)
                    val_s = amp * native_sin(freq * val_s);
                if (val_s > 1024) {
                    res[get_global_id(0)] = val_s;
                }
                if (val_d > 1024) {
                    res[get_global_id(0)] = (float)val_d;
                }
            }
            else {
